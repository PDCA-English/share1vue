...
export default {
  ...
  methods: {
      ...
      if (result) {
        this.shares[index].like.forEach((element) => {
          if (element.user_id == this.$store.state.user.id) {
            axios({
              method: "delete",
              url: "https://enigmatic-fortress-49360.herokuapp.com/api/like",
              data: {
        ...
        axios
          .post("https://enigmatic-fortress-49360.herokuapp.com/api/like", {
            share_id: this.shares[index].item.id,
            user_id: this.$store.state.user.id,
          })
          .then((response) => {
    ...
    del(index) {
      axios
        .delete(
          "https://enigmatic-fortress-49360.herokuapp.com/api/shares/" +
